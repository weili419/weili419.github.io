...
export default function HotelGuide(){
 return <section id="hotels" className="section hotel-section">
  ...
  <div className="hotel-timeline"><a href="#hotel-phuket"><small>01 / 9.20—24</small><b>普吉 · 4 晚</b><span>芭东连续入住</span></a><a href="#hotel-pattaya"><small>02 / 9.24—26</small><b>芭提雅 · 2 晚</b><span>曼谷机场直达</span></a><a href="#hotel-bangkok"><small>03 / 9.26—30</small><b>曼谷 · 4 晚</b><span>连住同一家，再飞回杭州</span></a></div>
  ...
  {cities.map(city=><div className={'hotel-city hotel-city-'+city.id} id={'hotel-'+city.id} key={city.id}>
   <div className="hotel-city-heading"><h3>{city.name}<span>{city.dates}{city.id==='bangkok'&&<> → 9/30 离店 · 4 晚</>}</span></h3><p>{city.text}</p></div>
  ...
  <div className="hotel-bottom-grid"><div className="quiet-note"><b>住宿规划：¥1,500／人</b><p>按三人分摊、平均 ¥450／间／晚作规划，共 10 晚；这不是上面房源的订单合计。Asok 若只剩高价套房，先比较位置与实际交通成本，再决定是否换区。</p></div><div className="quiet-note"><b>订单里要明确写下</b><p>3 adults, 3 separate single beds；如选双床加床，写明 2 twin beds + 1 adult extra bed，并核对加床费已计入。仅写「可住 3 人」不能保证有第三张床。「限时取消」需看具体日期、当地时间与扣费规则。</p><p>曼谷连续下单：9/26—30 共 4 晚。若凌晨或很早返程，先核航班，再决定最后一晚的住宿区域。</p></div></div>
 </section>;
}
